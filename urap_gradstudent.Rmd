...
library(ggplot2)
library(dplyr)
gen_data_raw <- read.csv("gradstudent_gender_ethnic_data.csv")
l2l3_data_raw <- read.csv("gradstudent_gender_ethnic_data_l2l3.csv")
gen_data <- data.frame(
...
## Multiracial (1)
multi.nonURM_female <- c(rep(0,14), gen_data$count[gen_data$ethnicity == '2 or More Races - Non-URM' & gen_data$gender == "Female"])
...
multi.URM_male <- c(rep(0,14), gen_data$count[gen_data$ethnicity == '2 or More Races - URM' & gen_data$gender == "Male"])
...
pacific_f <- c(rep(0,10), 1, 0, 1, 1, 1, 2, 3, 2, 1, 1, 1, 0)
pacific_m <- c(rep(0,15), 2, 3, 0, 1, 0, 0, 1)
...
vietnam_male <- c(rep(0,15), gen_data$count[gen_data$ethnicity == 'Vietnamese' & gen_data$gender == "Male"])
...
unknown_female <- c(0, gen_data$count[gen_data$ethnicity == 'Unknown' & gen_data$gender == "Female"], rep(0,8))
unknown_male <- c(rep(0,10), gen_data$count[gen_data$ethnicity == 'Unknown' & gen_data$gender == "Male"], rep(0,8))
...
## latinx (6)
hispanic_female <- c(rep(0,14), 1, 0, 6, 5, 6, 11, 7, 5)
hispanic_male <- c(rep(0,15), gen_data$count[gen_data$ethnicity == 'Hispanic' & gen_data$gender == "Male"])
...
n.african_female <- c(rep(0,14), gen_data$count[gen_data$ethnicity == 'North African' & gen_data$gender == "Female"])
...
asian_gender_graph <- ggplot(data = asian, aes(x = Date, y = Count, color = Gender)) + 
  geom_line(size = 1) + ggtitle("Asian Graduate Students, by Gender")
asian_gender_graph
ggsave("asian_women_gradstudent_gender.png")
...
aapi_gender_graph <- ggplot(data = aapi, aes(x = Date, y = Count, color = Gender)) + 
  geom_line(size = 1) + ggtitle("AAPI Graduate Students, by Gender")
...
women_race_graph <- ggplot(data = women, aes(x = Date, y = Count, color = Race)) + 
  geom_line(size = 1) + ggtitle("Female Graduate Students, by Race")
women_race_graph
...
ggsave("asian_women_gradstudent_ethnicity.png")
```
```{r}
